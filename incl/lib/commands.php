...
class Commands {
	...
	public static function doCommands($accountID, $comment, $levelID) {
		...
		if(substr($comment,0,5) == '!nocp' AND $gs->checkPermission($accountID, "commandRate")){
		...
		if(substr($comment,0,7) == '!unepic' AND $gs->checkPermission($accountID, "commandUnepic")){
			...
			$query = $db->prepare("INSERT INTO modactions (type, value, value3, timestamp, account) VALUES ('4', :value, :levelID, :timestamp, :id)");
			$query->execute([':value' => "0", ':timestamp' => $uploadDate, ':id' => $accountID, ':levelID' => $levelID]);
			PostToHook("Command - Feature", "$uname unsuper'd $aLevelName by $aUserName ($levelID).");
...
*/
		...
		if (str_starts_with($comment, '!send')) {
			// check if helper banned
			$query = $db->prepare("SELECT 1 FROM accounts WHERE accountID = :accID AND helperBanned=0");
			$query->execute([':accID' => $accountID]);
			if ($query->fetchColumn()) {
				// not helper banned!
				$diff = $commentarray[1];
				$stars = $commentarray[2];
				$feature = (int)$commentarray[3];
				...
				$query->execute([':account' => $accountID, ':level' => $levelID, ':diff' => $starDifficulty, ':stars' => $stars, ':feat' => $feature, ':auto' => $starAuto, ':demon' => $starDemon, ':timestamp' => time()]);
...
*/
		if(substr($comment,0,8) == '!noshare' AND (($gs->checkPermission($accountID, "commandSharecpAll") OR ($targetExtID == $accountID AND $gs->checkPermission($accountID, "commandSharecpOwn"))))){
