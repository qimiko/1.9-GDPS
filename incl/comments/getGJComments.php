...
foreach($result as &$comment1) {
	if($comment1["commentID"]!=""){
		...
		$selectedComment = base64_decode($comment1["comment"]);
		if (str_starts_with($selectedComment, '<cg>')) {
			$selectedComment = str_replace(array('<cg>', '</c>'), '', $selectedComment);
		}

		$commentText = ($gameVersion < 20) ? $selectedComment : base64_encode($selectedComment);
		if ($gameVersion < 20) {
		...
		if ($comment1['userName']) { //TODO: get rid of queries caused by getMaxValuePermission and getAccountCommentColor
			...
			if($binaryVersion > 31){
				...
				$commentstring .= "~11~$badge$colorString";
			}

			if ($binaryVersion <= 31 && !in_array($comment1["userID"], $users)){
